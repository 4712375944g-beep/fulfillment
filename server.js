...
app.post('/telegram-webhook', (req, res) => {
  ...
  if (text === '/start' || text === '/start@Sell_full_bot') {
    ...
    tg('sendMessage', {
      ...
      text: '🏭 *Фулфилмент — найдём склад для вашего товара*\n\n📋 *Как это работает:*\n1\. Выберите страну и город\n2\. Укажите способ доставки и площадку\n3\. Оставьте контакты — заявка уйдёт фулфилментам\n   Они сами свяжутся, а вы выберете лучшие условия\n\nНажмите кнопку ниже чтобы начать:',
      reply_markup: {
...
// /togglenotify — вкл/выкл уведомления для партнёра
  ...
  if (text === '/mystats' || text === '/mystats@Sell_full_bot') {
    ...
    if (userStats.role === 'partner') {
      var pOrders = (dbStats.orders || []).filter(function(o) { return o.city === userStats.city && (!userStats.zone || o.zone === userStats.zone); });
      var st = { new: 0, in_progress: 0, done: 0, cancelled: 0 };
      pOrders.forEach(function(o) { st[o.status] = (st[o.status] || 0) + 1; });
      tg('sendMessage', { chat_id: chatId, parse_mode: 'HTML', text: '📊 <b>Статистика фулфилмента</b>\n\n📍 ' + (userStats.city || '?') + (userStats.zone ? ' — ' + userStats.zone : '') + '\n\n📦 Всего заявок: ' + pOrders.length + '\n🆕 Новых: ' + (st.new || 0) + '\n🔄 В работе: ' + (st.in_progress || 0) + '\n✅ Выполнено: ' + (st.done || 0) + '\n❌ Отменено: ' + (st.cancelled || 0) });
    } else if (userStats.role === 'client') {
      var cOrders = (dbStats.orders || []).filter(function(o) { return o.phone === userStats.phone || o.phone === userStats.login; });
      tg('sendMessage', { chat_id: chatId, parse_mode: 'HTML', text: '📋 <b>Ваши заявки</b>\n\n📦 Всего: ' + cOrders.length + (cOrders.length === 0 ? '\nНет заявок' : '') });
      cOrders.slice(-5).forEach(function(o) { var se = { new: '🆕', in_progress: '🔄', done: '✅', cancelled: '❌' }; tg('sendMessage', { chat_id: chatId, parse_mode: 'HTML', text: [se[o.status] || '📦', '📍 ' + o.city + (o.zone ? ' — ' + o.zone : ''), '📦 ' + (o.method || 'FBO'), '📅 ' + o.created_at].join('  |  ') }); });
    }
    return;
  }
